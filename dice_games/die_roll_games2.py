...
def game3():
    print("Game 3: Play Game 2 multiple times and display a histogram of the results.")
    sides = int(input("How many sides? "))
    times = int(input("How many times to play? "))

    results = [game2(sides, silent=True) for _ in range(times)]

    plt.hist(results, bins=max(set(results), key=results.count), edgecolor='black')
    plt.title('Histogram of Rolls to Match Target Roll')
...
def main():
    choice = input("Which game would you like to play? Enter 1 for Game 1, 2 for Game 2, or 3 for Game 3: ")
    if choice == "1":
        game1()
    elif choice == "2":
        game2()
