...
### TODO: check back after 2026-09

**Path to retiring `--sabr` (2026-08-27):** The `--sabr`/`venv-sabr` split, the wpc PO token provider, and the [process-group cleanup added in v2.5.2](claude_summaries/chat-summary_2026-08-27.md) for orphaned Chrome instances it can leave behind all exist only because SABR support isn't in stable yt-dlp yet. Once PR #13515 merges, re-test: `pip install -U yt-dlp` in the main venv, run a normal (non-`--sabr`) download against the same VPN-exit IP that currently 403s, and if the full video downloads via SABR formats without the separate build, retire `venv-sabr`, `_sabr_ytdlp_path()`, the `--sabr` flag, and its README/CLAUDE.md setup docs. Still open even after the merge: whether stable yt-dlp's SABR handling still needs a browser-attested token provider like wpc, since mid-stream re-validation rejecting bgutil's synthetic tokens was the whole reason wpc was added (see above) — or whether that gets a first-party fix too.
